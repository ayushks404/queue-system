...
export async function getReportsSummary(req: Request, res: Response): Promise<void> {
  try {
    ...
    const summary = {
      ...
      avg_wait_time: Number(Number(waitTimeResult[0]?.avg_wait_minutes || 0).toFixed(4)),
      avg_service_time: Number(Number(serviceTimeResult[0]?.avg_service_minutes || 0).toFixed(4)),
